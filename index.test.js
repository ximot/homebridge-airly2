...
describe("homebridge-airly2", () => {
	...
	describe("validateMaxDistance", () => {
		...
		it("should cap at 50km and log warning", () => {
			...
			expect(mockThis.log.warn).toHaveBeenCalledWith(
				expect.stringContaining("capped at 50km"),
			);
		});
	...
	describe("normalizeMeasurement", () => {
		const createNormalizeMockThis = () => {
			const mockThis = createMockThis();
			mockThis.normalizeMeasurement =
				AirAccessory.prototype.normalizeMeasurement;
			return mockThis;
		};
		...
		it("should return first element of array", () => {
			...
			expect(mockThis.normalizeMeasurement(data)).toEqual({
				current: { values: [] },
			});
		});
	...
	describe("getSensorValue", () => {
		...
		it("should return null for invalid data", () => {
			...
			expect(
				getSensor.call(mockThis, { current: { values: "not-array" } }, "PM25"),
			).toBe(null);
		});
		...
		it("should return null if value is not a number", () => {
			const data = {
				current: { values: [{ name: "PM25", value: "invalid" }] },
			};
			expect(getSensor.call(mockThis, data, "PM25")).toBe(null);
		});
	});
	...
	describe("generateSerialNumber", () => {
		...
		it("should generate different serials for different locations", () => {
			const mockThis1 = createMockThis({
				latitude: 52.2297,
				longitude: 21.0122,
			});
			const mockThis2 = createMockThis({
				latitude: 50.0647,
				longitude: 19.945,
			});
	...
	describe("AirAccessory constructor", () => {
		...
		it("should throw error when apikey is missing", () => {
			...
			expect(() => new AirAccessory(mockLog, mockConfig, mockHbApi)).toThrow(
				"apikey is missing",
			);
		});
		...
		it("should fallback to AIRLY_CAQI for invalid indexType", () => {
			...
			expect(mockLog.warn).toHaveBeenCalledWith(
				expect.stringContaining("Unknown indexType"),
			);
		});
	});
	...
	describe("httpGetJson (via refreshMeasurement)", () => {
		...
		it("should make HTTPS request with correct headers", async () => {
			...
			const refreshPromise =
				AirAccessory.prototype.refreshMeasurement.call(mockThis);

			// Simulate response
			mockResponse.emit(
				"data",
				JSON.stringify({ current: { indexes: [{ value: 50 }] } }),
			);
	...
	describe("updateData", () => {
		it("should return 0 for invalid measurement", () => {
			...
			const result = AirAccessory.prototype.updateData.call(
				mockThis,
				null,
				"Fetch",
			);
		...
		it("should return 0 when data has no current property", () => {
			const mockThis = createMockThis();
			mockThis.getSensorValue = AirAccessory.prototype.getSensorValue;
			mockThis.getIndexValue = AirAccessory.prototype.getIndexValue;
			...
			expect(result).toBe(0);
			expect(mockThis.log.warn).toHaveBeenCalledWith(
				expect.stringContaining("does not contain current measurement data"),
			);
		});
	});

	describe("getServices", () => {
		it("should return array with AccessoryInformation and AirQualitySensor services", () => {
			const mockHbApi = createMockHomebridgeApi();
			pluginInit(mockHbApi);
		...
		it("should set manufacturer, model, and serial number on info service", () => {
			const mockHbApi = createMockHomebridgeApi();
			pluginInit(mockHbApi);
			...
			expect(infoService.setCharacteristic).toHaveBeenCalledWith(
				"SerialNumber",
				expect.stringMatching(/^AIR2-/),
			);
		});

		it("should register onGet handler for AirQuality characteristic", () => {
			const mockHbApi = createMockHomebridgeApi();
			pluginInit(mockHbApi);
			...
			const accessory = new AirAccessory(mockLog, mockConfig, mockHbApi);
			const services = accessory.getServices();
			const airService = services[1];

			// getCharacteristic should have been called
			expect(airService.getCharacteristic).toHaveBeenCalledWith("AirQuality");
		});

		it("should fallback to on(get) when onGet is not available", () => {
			const mockHbApi = createMockHomebridgeApi();
			// Override AirQualitySensor to return characteristic without onGet
			const onHandler = jest.fn();
			mockHbApi.hap.Service.AirQualitySensor = jest
				.fn()
				.mockImplementation(() => ({
					getCharacteristic: jest.fn().mockReturnValue({
						on: onHandler,
			...
			expect(onHandler).toHaveBeenCalledWith("get", expect.any(Function));
		});

		it("should add NO2 and O3 characteristics", () => {
			const mockHbApi = createMockHomebridgeApi();
			pluginInit(mockHbApi);
			...
			expect(airService.addCharacteristic).toHaveBeenCalledWith(
				"NitrogenDioxideDensity",
			);
			expect(airService.addCharacteristic).toHaveBeenCalledWith("OzoneDensity");
		});
	});

	describe("httpGetJson edge cases (via refreshMeasurement)", () => {
		beforeEach(() => {
			jest.clearAllMocks();
		});
		...
		it("should reject on request timeout", async () => {
			...
			mockRequest.setTimeout = jest.fn((ms, cb) => {
				// Immediately trigger timeout
				cb();
			});

			https.request.mockImplementation((url, options, callback) => {
				// Don't call callback - simulating hanging request
				return mockRequest;
			});

			const mockThis = createMockThis({ refreshPromise: null });
			mockThis.buildApiUrl = AirAccessory.prototype.buildApiUrl;
		...
		it("should reject on invalid JSON response", async () => {
			...
			const promise = AirAccessory.prototype.refreshMeasurement.call(mockThis);
		...
		it("should handle error path without HapStatusError globals", async () => {
			const mockResponse = new EventEmitter();
			mockResponse.statusCode = 500;
			mockResponse.setEncoding = jest.fn();
			...
			// Create a special mock API without HapStatusError/HAPStatus
			const mockHbApiNoHap = createMockHomebridgeApi();
			mockHbApiNoHap.hap.HapStatusError = null;
			mockHbApiNoHap.hap.HAPStatus = null;
			pluginInit(mockHbApiNoHap);
			...
			await expect(promise).rejects.toThrow("Airly API request failed");
			expect(mockThis.log.error).toHaveBeenCalled();

			// Restore HapStatusError for other tests
			const restoreApi = createMockHomebridgeApi();
			pluginInit(restoreApi);
		});
	});
	...
	describe("Polling edge cases", () => {
		...
		it("should handle refresh errors gracefully in polling callback", () => {
			const mockThis = createMockThis({
				refreshIntervalSeconds: 900,
				api: { on: jest.fn() },
			});
			// Both calls return promises - first resolves, second rejects
			mockThis.refreshMeasurement = jest
				.fn()
				.mockReturnValue(Promise.resolve())
				.mockReturnValueOnce(Promise.resolve()) // initial call
				.mockReturnValueOnce(Promise.reject(new Error("network fail"))); // interval call

			AirAccessory.prototype.startPolling.call(mockThis);

			// Trigger the interval callback — error is caught by handleError
			jest.advanceTimersByTime(900 * 1000);

			expect(mockThis.refreshMeasurement).toHaveBeenCalledTimes(2);
		});

		it("should handle refresh errors with undefined message", () => {
			const mockThis = createMockThis({
				refreshIntervalSeconds: 900,
				api: { on: jest.fn() },
			});
			mockThis.refreshMeasurement = jest
				...
				.mockReturnValueOnce(Promise.reject({})); // error without message
	...
	describe("on(get) callback handler", () => {
		it("should resolve callback with value on success", async () => {
			const mockHbApi = createMockHomebridgeApi();
			const onHandler = jest.fn();
			mockHbApi.hap.Service.AirQualitySensor = jest
				.fn()
			...
			// Get the callback that was registered with on('get', cb)
			const getCallback = onHandler.mock.calls[0][1];
			const callback = jest.fn();

			// Mock getAirData to return a value
			accessory.cache = { current: { indexes: [{ value: 50 }] } };
			accessory.getAirData = jest.fn().mockResolvedValue(2);
		...
		it("should pass error to callback on failure", async () => {
			...
			accessory.getAirData = jest.fn().mockRejectedValue(testError);
